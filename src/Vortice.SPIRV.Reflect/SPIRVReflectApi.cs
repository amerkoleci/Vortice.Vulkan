...
unsafe partial class SPIRVReflectApi
{
    ...
    public static ReadOnlySpan<SpvReflectDescriptorBinding> spvReflectEnumerateDescriptorBindings(SpvReflectShaderModule* module)
    {
        uint count = 0;
        spvReflectEnumerateDescriptorBindings(module, &count, null).CheckResult();

        ReadOnlySpan<SpvReflectDescriptorBinding> result = new SpvReflectDescriptorBinding[count];
        fixed (SpvReflectDescriptorBinding* resultPtr = result)
        {
            spvReflectEnumerateDescriptorBindings(module, &count, resultPtr).CheckResult();
        }
        return result;
    }

    public static ReadOnlySpan<SpvReflectDescriptorBinding> spvReflectEnumerateEntryPointDescriptorBindings(SpvReflectShaderModule* module, string entryPoint)
    {
        return spvReflectEnumerateEntryPointDescriptorBindings(module, entryPoint.GetUtf8Span());
    }

    public static ReadOnlySpan<SpvReflectDescriptorBinding> spvReflectEnumerateEntryPointDescriptorBindings(SpvReflectShaderModule* module, ReadOnlySpan<sbyte> entryPoint)
    {
        fixed (sbyte* pEntryPoint = entryPoint)
        {
            uint count = 0;
            spvReflectEnumerateEntryPointDescriptorBindings(module, pEntryPoint, &count, null).CheckResult();

            ReadOnlySpan<SpvReflectDescriptorBinding> result = new SpvReflectDescriptorBinding[count];
            fixed (SpvReflectDescriptorBinding* resultPtr = result)
            {
                spvReflectEnumerateEntryPointDescriptorBindings(module, pEntryPoint, &count, resultPtr).CheckResult();
            }
            return result;
        }
    }

    public static ReadOnlySpan<SpvReflectDescriptorSet> spvReflectEnumerateDescriptorSets(SpvReflectShaderModule* module)
    {
        uint count = 0;
        spvReflectEnumerateDescriptorSets(module, &count, null).CheckResult();

        ReadOnlySpan<SpvReflectDescriptorSet> result = new SpvReflectDescriptorSet[count];
        fixed (SpvReflectDescriptorSet* resultPtr = result)
        {
            spvReflectEnumerateDescriptorSets(module, &count, resultPtr).CheckResult();
        }
        return result;
    }

    public static ReadOnlySpan<SpvReflectDescriptorSet> spvReflectEnumerateEntryPointDescriptorSets(SpvReflectShaderModule* module, string entryPoint)
    {
        return spvReflectEnumerateEntryPointDescriptorSets(module, entryPoint.GetUtf8Span());
    }

    public static ReadOnlySpan<SpvReflectDescriptorSet> spvReflectEnumerateEntryPointDescriptorSets(SpvReflectShaderModule* module, ReadOnlySpan<sbyte> entryPoint)
    {
        fixed (sbyte* pEntryPoint = entryPoint)
        {
            uint count = 0;
            spvReflectEnumerateEntryPointDescriptorSets(module, pEntryPoint, &count, null).CheckResult();

            ReadOnlySpan<SpvReflectDescriptorSet> result = new SpvReflectDescriptorSet[count];
            fixed (SpvReflectDescriptorSet* resultPtr = result)
            {
                spvReflectEnumerateEntryPointDescriptorSets(module, pEntryPoint, &count, resultPtr).CheckResult();
            }
            return result;
        }
    }

    public static ReadOnlySpan<SpvReflectBlockVariable> spvReflectEnumeratePushConstantBlocks(SpvReflectShaderModule* module)
    {
        uint count = 0;
        spvReflectEnumeratePushConstantBlocks(module, &count, null).CheckResult();

        ReadOnlySpan<SpvReflectBlockVariable> result = new SpvReflectBlockVariable[count];
        fixed (SpvReflectBlockVariable* resultPtr = result)
        {
            spvReflectEnumeratePushConstantBlocks(module, &count, resultPtr).CheckResult();
        }
        return result;
    }

    public static ReadOnlySpan<SpvReflectBlockVariable> spvReflectEnumerateEntryPointPushConstantBlocks(SpvReflectShaderModule* module, string entryPoint)
    {
        return spvReflectEnumerateEntryPointPushConstantBlocks(module, entryPoint.GetUtf8Span());
    }

    public static ReadOnlySpan<SpvReflectBlockVariable> spvReflectEnumerateEntryPointPushConstantBlocks(SpvReflectShaderModule* module, ReadOnlySpan<sbyte> entryPoint)
    {
        fixed (sbyte* pEntryPoint = entryPoint)
        {
            uint count = 0;
            spvReflectEnumerateEntryPointPushConstantBlocks(module, pEntryPoint, &count, null).CheckResult();

            ReadOnlySpan<SpvReflectBlockVariable> result = new SpvReflectBlockVariable[count];
            fixed (SpvReflectBlockVariable* resultPtr = result)
            {
                spvReflectEnumerateEntryPointPushConstantBlocks(module, pEntryPoint, &count, resultPtr).CheckResult();
            }
            return result;
        }
    }
